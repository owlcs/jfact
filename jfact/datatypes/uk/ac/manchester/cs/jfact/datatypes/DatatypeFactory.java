...
public final class DatatypeFactory {
	...
	static abstract class ABSTRACT_NUMERIC_DATATYPE<R extends Comparable<R>> extends
		...
		public boolean isInValueSpace(final R l) {
			if (this.hasMinExclusive()) {
				if (this.getMin().compareTo(minExclusive.parseNumber(l)) <= 0) {
					return false;
				}
			}
			if (this.hasMinInclusive()) {
				if (this.getMin().compareTo(minInclusive.parseNumber(l)) < 0) {
					return false;
				}
			}
			if (this.hasMaxExclusive()) {
				if (this.getMax().compareTo(maxExclusive.parseNumber(l)) >= 0) {
					return false;
				}
			}
			if (this.hasMaxInclusive()) {
				if (this.getMax().compareTo(maxInclusive.parseNumber(l)) > 0) {
					return false;
				}
			}
			return true;
	...
	static class DATETIME_DATATYPE extends ABSTRACT_DATATYPE<Calendar> implements
		...
		public Calendar getMin() {
			if (hasMinExclusive()) {
				return this.getFacetValue(minExclusive);
			}
			if (hasMinInclusive()) {
				return this.getFacetValue(minInclusive);
			}
			return null;
		}
		...
		public Calendar getMax() {
			if (hasMaxExclusive()) {
				return this.getFacetValue(maxExclusive);
			}
			if (hasMaxInclusive()) {
				return this.getFacetValue(maxInclusive);
			}
			return null;
		}
	}
