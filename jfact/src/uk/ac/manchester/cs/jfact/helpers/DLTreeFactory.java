...
public final class DLTreeFactory {
    ...
    public static DLTree createInverse(final DLTree R) {
        assert R != null;
        if (R.token() == INV) {
    ...
    static boolean isSemanticallyDataTop(final DLTree dr) {
    ...
    static boolean isSemanticallyDataBottom(final DLTree dr) {
    ...
    static boolean isDataRangeBigEnough(final DLTree dr, final int n) {
        return true;
    }

    /// simplify universal restriction with top data role
    static DLTree simplifyDataTopForall(final DLTree dr) {
    ...
    static DLTree simplifyDataTopLE(final int n, final DLTree dr) {
    ...
    public static DLTree buildDisjAux(final List<DLTree> beg) {
    ...
    public static DLTree createSNFAnd(final DLTree C, final DLTree D) {
        if (C == null) {
            return D;
        }
        if (D == null) {
    ...
    public static DLTree createSNFAnd(final Collection<DLTree> collection) {
    ...
    public static DLTree createSNFExists(final DLTree R, final DLTree C) {
        // \ER.C . \not\AR.\not C
        return createSNFNot(createSNFForall(R, createSNFNot(C)));
    }

    public static DLTree createSNFForall(final DLTree R, final DLTree C) {
        if (C.isTOP()) {
            return C;
        } else if (isBotRole(R)) {
            return createTop();
    ...
    public static DLTree createRole(final Role R) {
        return createEntry(R.isDataRole() ? Token.DNAME : Token.RNAME, R);
    }

    public static DLTree createEntry(final Token tag, final NamedEntry entry) {
        return new LEAFDLTree(new Lexeme(tag, entry));
    }

    /** create at-most (LE) restriction of given formulas (<= n R.C) */
    public static DLTree createSNFLE(final int n, final DLTree R, final DLTree C) {
    ...
    public static DLTree createSNFGE(final int n, final DLTree R, final DLTree C) {
        if (n == 0) {
            return createTop();
        }
        if (C.isBOTTOM()) {
    ...
    public static DLTree createSNFNot(final DLTree C) {
        assert C != null;
        if (C.isBOTTOM()) {
            // \not F = T
            return createTop();
    ...
    public static DLTree createSNFNot(final DLTree C, final DLTree ancestor) {
        assert C != null;
        if (C.isBOTTOM()) {
            // \not F = T
            return createTop();
    ...
    public static DLTree createSNFOr(final Collection<DLTree> C) {
        // C\or D . \not(\not C\and\not D)
    ...
    public static DLTree wrap(final NamedEntry t) {
    ...
    public static DLTree buildTree(final Lexeme t, final DLTree t1, final DLTree t2) {
        return new TWODLTree(t, t1, t2);
    }

    public static DLTree buildTree(final Lexeme t, final DLTree t1) {
        return new ONEDLTree(t, t1);
    }

    public static DLTree buildTree(final Lexeme t) {
    ...
    public static boolean replaceSynonymsFromTree(final DLTree desc) {
        if (desc == null) {
            return false;
        }
        if (desc.isName()) {
