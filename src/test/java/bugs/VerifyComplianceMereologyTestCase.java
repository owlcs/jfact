...
class VerifyComplianceMereologyTestCase extends VerifyComplianceBase {
    ...
    @Test
    void shouldPassgetSuperClassesWholefalse() {
        equal(reasoner.getSuperClasses(Whole, false), Thing, Abstract_Entity);
    }

    @Test
    void shouldPassgetSubClassesAbstract_Entityfalse() {
        equal(reasoner.getSubClasses(Abstract_Entity, false), Whole, Pair, Composition, Atom,
            Nothing, Part);
    }

    @Test
    void shouldPassgetSubClassesAbstract_Entitytrue() {
        equal(reasoner.getSubClasses(Abstract_Entity, true), Whole, Atom, Part);
    }

    @Test
    void shouldPassgetSuperClassesPairfalse() {
        equal(reasoner.getSuperClasses(Pair, false), Thing, Whole, Abstract_Entity, Composition);
    }

    @Test
    void shouldPassgetSuperClassesCompositionfalse() {
        equal(reasoner.getSuperClasses(Composition, false), Thing, Whole, Abstract_Entity);
    }

    @Test
    void shouldPassgetSuperClassesPartfalse() {
        equal(reasoner.getSuperClasses(Part, false), Thing, Abstract_Entity);
    }

    @Test
    void shouldPassgetSubClassesThingtrue() {
        equal(reasoner.getSubClasses(Thing, true), Physical_Entity, Abstract_Entity, Occurrence,
            Mental_Entity);
    }

    @Test
    void shouldPassgetSuperClassesWholetrue() {
        equal(reasoner.getSuperClasses(Whole, true), Abstract_Entity);
    }

    @Test
    void shouldPassgetSuperClassesParttrue() {
        equal(reasoner.getSuperClasses(Part, true), Abstract_Entity);
    }

    @Test
    void shouldPassisEntailedSubClassOfCompositionAbstract_Entity() {
        assertTrue(reasoner.isEntailed(df.getOWLSubClassOfAxiom(Composition, Abstract_Entity)));
    }

    @Test
    void shouldPassisEntailedSubClassOfPairAbstract_Entity() {
        assertTrue(reasoner.isEntailed(df.getOWLSubClassOfAxiom(Pair, Abstract_Entity)));
    }
}
