...
public class AddIndividualsAfterLoadingTestCase extends TestBase {
    ...
    public void shouldLoadAndNotFailQueryDataHasValue() throws Exception {
        ...
        OWLOntologyManager m = o.getOWLOntologyManager();
        OWLDataFactory f = m.getOWLDataFactory();
        OWLDataProperty p = f.getOWLDataProperty("urn:test#p");
        OWLDatatype floatType = f.getFloatOWLDatatype();
        OWLNamedIndividual i = f.getOWLNamedIndividual("urn:test#i");
        o.add(f.getOWLDataPropertyRangeAxiom(p, floatType));
        o.add(f.getOWLDataPropertyAssertionAxiom(p, i, 19.0F));
        OWLFacetRestriction fLess20 = f.getOWLFacetRestriction(OWLFacet.MAX_INCLUSIVE, 20f);
        OWLReasoner r = factory().createReasoner(o);
        r.precomputeInferences(InferenceType.CLASS_HIERARCHY);
        OWLDatatypeRestriction lessThan20 = f.getOWLDatatypeRestriction(floatType, fLess20);
        NodeSet<OWLNamedIndividual> instances = r.getInstances(f.getOWLDataAllValuesFrom(p, lessThan20), false);
