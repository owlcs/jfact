...
public class JFactReasoner implements OWLReasoner, OWLOntologyChangeListener, OWLKnowledgeExplorerReasoner,
    ...
    public static Collection<OWLAxiom> importsIncluded(OWLOntology ont) {
        ...
        List<OWLOntology> listOfImports = new ArrayList<>(ont.getImportsClosure());
        for (AxiomType<?> t : types) {
            for (OWLOntology o : listOfImports) {
                axioms.addAll(o.getAxioms(t));
            }
        }
        return axioms;
    }
    ...
    public synchronized Node<OWLClass> getEquivalentClasses(OWLClassExpression ce) {
        ...
        if (!isFreshName(ce)) {
            checkConsistency();
            pointers = kernel.getEquivalentConcepts(tr.pointer(ce), classActor()).getSynonyms();
        }
    ...
    public synchronized NodeSet<OWLClass> getSuperClasses(OWLClassExpression ce, boolean direct) {
        ...
        checkConsistency();
        return tr.getClassExpressionTranslator().nodeSet(askSuperClasses(tr.pointer(ce), direct));
    }

    @Override
    public synchronized NodeSet<OWLClass> getDisjointClasses(OWLClassExpression ce) {
        ConceptExpression p = tr.pointer(ce);
        return tr.getClassExpressionTranslator().nodeSet(kernel.getDisjointConcepts(p, classActor()).getElements());
    }
    ...
    public synchronized NodeSet<OWLObjectPropertyExpression> getSubObjectProperties(OWLObjectPropertyExpression pe,
        boolean direct) {
        checkConsistency();
        return tr.getObjectPropertyTranslator().nodeSet(kernel.getRoles(tr.pointer(pe), direct, objectActor(), false)
            .getElements());
    }
    ...
    public synchronized Node<OWLObjectPropertyExpression> getEquivalentObjectProperties(
        ...
        checkConsistency();
        return tr.getObjectPropertyTranslator().node(kernel.getEquivalentRoles(tr.pointer(pe), objectActor())
            .getSynonyms());
    }
    ...
    public synchronized NodeSet<OWLClass> getObjectPropertyDomains(OWLObjectPropertyExpression pe, boolean direct) {
        checkConsistency();
        return tr.getClassExpressionTranslator().nodeSet(kernel.getORoleDomain(tr.pointer(pe), direct, classActor())
            .getElements());
    }
    ...
    public synchronized NodeSet<OWLDataProperty> getSubDataProperties(OWLDataProperty pe, boolean direct) {
        checkConsistency();
        return tr.getDataPropertyTranslator().nodeSet(kernel.getRoles(tr.pointer(pe), direct, dataActor(), false)
            .getElements());
    }
    ...
    public synchronized Node<OWLDataProperty> getEquivalentDataProperties(OWLDataProperty pe) {
        ...
        Collection<DataRoleExpression> dataPropertySynonyms = kernel.getEquivalentRoles(p, dataActor()).getSynonyms();
    ...
    public NodeSet<OWLClass> getDataPropertyDomains(OWLDataProperty pe, boolean direct) {
        return tr.getClassExpressionTranslator().nodeSet(kernel.getDRoleDomain(tr.pointer(pe), direct, classActor())
            .getElements());
    }
    ...
    public synchronized NodeSet<OWLClass> getTypes(OWLNamedIndividual ind, boolean direct) {
        checkConsistency();
        Collection<Collection<ConceptExpression>> classElements = kernel.getTypes(tr.pointer(ind), direct, classActor())
            .getElements();
    ...
    public synchronized Node<OWLNamedIndividual> getSameIndividuals(OWLNamedIndividual ind) {
        checkConsistency();
        return tr.getIndividualTranslator().node(kernel.getSameAs(tr.pointer(ind), individualActor(
            IndividualName.class)).getSynonyms());
    }
    ...
    public Node<? extends OWLObjectPropertyExpression> getObjectNeighbours(RootNode object, boolean deterministicOnly) {
        return tr.getObjectPropertyTranslator().node(kernel.getObjectRoles((DlCompletionTree) object.getNode(),
            deterministicOnly, false));
    }

    @Override
    public Node<OWLDataProperty> getDataNeighbours(RootNode object, boolean deterministicOnly) {
        return tr.getDataPropertyTranslator().node(kernel.getDataRoles((DlCompletionTree) object.getNode(),
            deterministicOnly));
    }
    ...
    public Collection<RootNode> getObjectNeighbours(RootNode n, OWLObjectProperty property) {
        ...
        for (DlCompletionTree t : kernel.getNeighbours((DlCompletionTree) n.getNode(), tr.pointer(property))) {
    ...
    public Node<? extends OWLClassExpression> getObjectLabel(RootNode object, boolean deterministicOnly) {
        Node<OWLClass> nodeFromPointers = tr.getClassExpressionTranslator().node(kernel.getObjectLabel(
    ...
    public Node<? extends OWLDataRange> getDataLabel(RootNode object, boolean deterministicOnly) {
        return tr.getDataRangeTranslator().node(kernel.getDataLabel((DlCompletionTree) object.getNode(),
            deterministicOnly));
    }
     ...
     */
    public synchronized Node<OWLNamedIndividual> getDataRelatedIndividuals(Set<OWLIndividual> individuals,
